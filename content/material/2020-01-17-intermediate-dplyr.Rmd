...
In this lesson we assume a bit of familiarity with the basic functions of dplyr (especially group-by() and `summarize()`), which we covered in the [intro_dplyr lesson](https://au-cru.github.io/site/material/2019-11-15-intro-dplyr/) 
...
The tidyverse, provides some very powerful packages (e.g. dplyr, tidyr) that can allow for complete reorganization of our data in a few lines of codes. Using these packages and their functions has several advantages:
...
becomes a different variable (i.e. column), under which the population numbers, 
...
a wider dataframe with more columns (the single countries), and less rows.
...
#### Exercise 1
Use the dataframe `us_rent_income` for this exercise. This dataframe shows the estimate value and the 90%
...
We can also do  the opposite, as shift from a dataframe in wide format to a dataframe in long format. 
...
- the argument `values_to`: the name of new variable under which we want to group the values of our variables.
...
There is one variable in the dataframe `population_wide` that we have not dealt with: `year`. 

We do not want to change the arrangement of this variable, and R detects that this is a different variable 
from the other columns that we want to reshape but not knowing what to do with it, it throws an error. 

So, we need to specify that the variable `year` is not to be involved in the reshaping.
