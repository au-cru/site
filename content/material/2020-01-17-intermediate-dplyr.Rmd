...
*Img: https://github.com/gadenbuie/tidyexplain*
...
We could select every column, with the [tidyselect](https://r4ds.had.co.nz/transform.html#select) helper function `everything()`. 
...
This breaks the row-wise relation between year and population, and is clearly not what we are want. Instead, we specify that the variable `year` is not to be involved in the reshaping. Columns not involved in the reshape will be copied, to preserve the row-wise relations in the data. 
...
So far, we have only reshaped "clean" datasets, but at major usecase for the `pivot_**` functions is tidying up messy data. In the example below, we will tidy a made up (but very realistic) table.
...
The main problem in this table is that the variable names also inclue a value (week number). We want to pivot this wide dataframe into a longer one, with each row containg an id, a week number, a systolic and a diastolic blood pressure and a weigth. For this we use `pivot_longer()`.

We do not want to include `id` in the reshape, so we select every column except this with `-id` as the first argument. We then seperate the variable names (e.g. 'bp_Dia_week1') into the actual variable name ( 'bp_Dia') and the value of the week variable ('1'). This is donw by setting the `names_sep` parameter to the string that seperates the variable name from the week value (here '_week'). Because we split each variable name into two, we must set `names_to` for both. We do this with the character vector `c('.value', 'week')`. The special value `'.value'` indicates that the corresponding substring of the original variable name will itself become a variable name for the values in the new dataframe (This makes a lot more sense when you look at the output of the function). The parameter `names_ptypes` convertes the variable `week` from a character to an integer.

```{r}
blood_pressure_long <- blood_pressure %>% 
  pivot_longer(-id, names_sep = '_week', names_to = c('.value', 'week'), names_ptypes = list(week = integer()))

blood_pressure_long
```

In some cases you may want only on column with blood pressures, and a seperate column indicating if the pressue is diastolic or systolic. We could pivot our `blood_pressure_long` even longer, but for the sake of demonstration, we will do it in one function call. As `weight` cannot be seperated in diastolic and systolic, we will first exclude this variable. 
